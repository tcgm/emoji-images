...
export default function LibrarySelector({ selected, setQuery }: LibrarySelectorProps) {
    return (
        <Box overflowY="auto" border="1px solid #ccc" borderRadius="md" p={2}>
                ...
                {LIBRARIES.map(lib => (
                    <Tooltip key={lib.value} label={lib.label} placement="top">
                        <IconButton
                            ...
                            icon={
                                lib.icon ? (
                                    <Icon as={lib.icon} boxSize={6} /> // Ensure proper sizing for Chakra UI icons
                                ) : (
